...
const Index = () => {
  const { avatarState, dailyScore, streak, points, level, diabetesRisk } =
    useUser();
  ...
  function startWaterReminder() {
    window.ReactNativeWebView?.postMessage("START_WATER_REMINDER");
  }
  ...
  return (
    <div className="min-h-screen bg-background pb-24 px-4 pt-6">
      <div className="mx-auto max-w-md space-y-5">
          ...
          <div className={`rounded-2xl ${riskBg} p-4`}>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-lg font-bold ${riskColor}`}>
                {riskEmoji}{" "}
                {diabetesRisk.level.charAt(0).toUpperCase() +
                  diabetesRisk.level.slice(1)}
              </span>
            </div>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {diabetesRisk.level === "low" &&
                "Your habits look great! Keep up the awesome work 💪"}
              {diabetesRisk.level === "medium" &&
                "Some factors to watch. Small changes can make a big difference! 💛"}
              {diabetesRisk.level === "high" &&
                "Your habits show increased risk. Let's fix it early 💙"}
            </p>
          </div>
          <p className="text-xs text-muted-foreground mt-2">
            ⚠️ Based on habits, not a medical diagnosis
          </p>
        </motion.div>
        ...
        <button
          onClick={startWaterReminder}
          className="flex items-center gap-3 rounded-2xl gradient-primary p-4 text-left text-primary-foreground shadow-card"
        >
          <span className="text-2xl">💧</span>
          <div>
            <span className="text-sm font-bold">Water Reminder</span>
            <p className="text-xs opacity-80">Every 2 minutes</p>
          </div>
        </button>
      </div>
    </div>
