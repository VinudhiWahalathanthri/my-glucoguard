...
const steps = [
  { title: 'Welcome! 🎉', subtitle: "Let's set up your GlucoGuard profile", field: 'welcome' },
  { title: 'How old are you?', subtitle: 'This helps us personalize your experience', field: 'age' },
  { title: "What's your gender?", subtitle: 'Select what applies to you', field: 'gender' },
  { title: "What's your height?", subtitle: 'In centimeters', field: 'height' },
  { title: "What's your weight?", subtitle: 'In kilograms', field: 'weight' },
  { title: 'Family history of diabetes?', subtitle: 'This is private & helps assess your risk', field: 'familyDiabetes' },
  { title: 'Daily sugar intake?', subtitle: 'How much sugar do you consume daily?', field: 'dailySugar' },
  { title: "You're all set! 🚀", subtitle: "Let's start your health journey", field: 'complete' },
];
        ...
        return (
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="flex gap-4">
              {[
                { icon: Heart, label: 'Track Health', color: 'text-destructive' },
                { icon: Shield, label: 'Stay Safe', color: 'text-primary' },
                { icon: Sparkles, label: 'Earn Rewards', color: 'text-accent' },
              ].map(({ icon: Icon, label, color }) => (
                <div key={label} className="flex flex-col items-center gap-2 rounded-2xl bg-card p-4 shadow-card">
            ...
            <p className="text-muted-foreground text-center max-w-xs">
              Your personal health guardian. Fun, private, and made just for teens! 💙
            </p>
          </div>
        );
      case 'age':
        ...
        return (
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex items-end gap-2">
              ...
              />
              <span className="pb-4 text-lg font-semibold text-muted-foreground">cm</span>
            </div>
          </div>
        );
      case 'weight':
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex items-end gap-2">
              <input
                ...
                className="w-36 rounded-2xl border-2 border-primary/20 bg-card px-6 py-4 text-center text-2xl font-bold text-foreground outline-none focus:border-primary transition-colors"
              />
              <span className="pb-4 text-lg font-semibold text-muted-foreground">kg</span>
            </div>
          </div>
        );
      case 'familyDiabetes':
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                >
                  ...
                  <span className={`text-xs ${profile.dailySugar === value ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}>{desc}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        return (
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="rounded-2xl gradient-primary p-6 text-primary-foreground text-center">
              ...
              <p className="text-sm opacity-90 mt-1">Time to start earning points & keeping healthy 💪</p>
            </div>
          </div>
        );
      default:
    ...
    <div className="flex min-h-screen flex-col items-center justify-between bg-background px-6 py-8">
      ...
      <div className="flex flex-1 flex-col items-center justify-center w-full max-w-md">
        <AnimatePresence mode="wait">
          ...
          >
            <h1 className="text-2xl font-bold text-foreground text-center">{currentStep.title}</h1>
      ...
      <div className="flex w-full max-w-md items-center justify-between gap-4">
        ...
        >
          {step === steps.length - 1 ? "Let's Go!" : 'Next'} <ArrowRight className="ml-2 h-4 w-4" />
        </Button>
      </div>
    </div>
  );
